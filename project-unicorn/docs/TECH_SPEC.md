...
**Three clauses above are still inaccurate as written and are flagged rather than quietly rewritten (2026-08-08).** The dropdown is a curated table *plus* the detected native mode, not a pure OS enumeration. The shipped UI-scale ladder is 75/90/100/110/125/150 percent, not 80 to 200, and its ends are gated — below 100 percent by a 9-pixel readability floor, above it by the 1920-wide design width, since the §14.4 breakpoints below do not exist yet. There is **no frame-rate cap** in the codebase. Reconciling this section is a spec decision, not an implementation detail.
