...
The primary design target is 1920 by 1080. The game supports the full range of standard modern resolutions: 1280 by 720 as the minimum, through 1366 by 768, 1600 by 900, 1680 by 1050, 1920 by 1080, 1920 by 1200, 2560 by 1080 ultrawide, 2560 by 1440, 2560 by 1600, 3440 by 1440 ultrawide, 3840 by 1600, 3840 by 2160, 3840 by 2400, and 5120 by 1440 super-ultrawide. Supported aspect ratios are 16:9 as primary, plus 16:10, 21:9, and 32:9, with the interface letterboxing or anchoring panels to edges on the widest ratios.

**The table is a floor, not the whole story (2026-08-08).** The monitor's own native mode is detected at runtime and inserted into the list, so panels outside this table — laptop 2880 by 1800, 3024 by 1964 and similar — are selectable too. The detected native, taskbar-aware, is also the first-run default; see the Decision Log entry. The ODA room is authored at 16:9 and carries a crop budget beyond which it is drawn as a centred band rather than cropped, which is the concrete form the "letterboxing on the widest ratios" clause above takes.
...
**Three clauses above are still inaccurate as written and are flagged rather than quietly rewritten (2026-08-08).** The dropdown is a curated table *plus* the detected native mode, not a pure OS enumeration. The shipped UI-scale ladder is 75/90/100/110/125 percent, not 80 to 200, and its ends are gated — below 100 percent by a 9-pixel readability floor, above it by the 1920-wide design width, since the §14.4 breakpoints below do not exist yet. (150 percent was removed 2026-08-18: `content_scale_factor` shrinks the logical viewport, so 150 percent on a 1080p panel reported 1280×720 and the Settings modal's footer — with its close button — fell off the bottom edge. The ladder is now capped at the tightest step the shell's own modals survive, and `endgame_smoke`'s `ui_scale_ladder_fits_settings` case holds that line.) There is **no frame-rate cap** in the codebase. Reconciling this section is a spec decision, not an implementation detail.
...
### 19.7. Delivery and version control

Director ruling 2026-08-20, and it supersedes the branch-per-round practice recorded in the
§20 Decision Log entry of 2026-08-19.

One checkout, one branch. Work is committed directly to `main` in the single working tree; no
feature branch, no git worktree, no merge commit, no rebase. The remote carries `main` and
nothing else — GitHub is how the work is reached from a second machine, not a second build
repository. If a branch or a second folder genuinely seems necessary the agent stops and asks,
the same way §19.2 requires for an ambiguous spec.

Review is the running game, not a diff. The agent changes files and Erdem opens the game.
"Geri al" reverts the change; "push et" publishes it. Nothing is pushed unasked. This is why
no branch is needed to hold work pending approval — a revert does that job, and it does it
without splitting the tree the game is played from.

Per-commit obligations are unchanged (bilingual strings, `THEME_STAMP` atomicity, gates green
before the commit that claims them). See `CLAUDE.md` — DELIVERY LAW.
...
| 2026-07-29 | **Game speed ladder retuned + 3x added.** §8.1's "1 real second = 1 in-game hour" rule is retired. Tempo is now expressed as **real seconds per in-game day** in a single table, `TimeManager.SECONDS_PER_DAY = [0.0, 12.0, 6.0, 3.0, 1.5]` (idx 0=pause, 1=1x, 2=2x, 3=3x, 4=4x) — a 2× speed-up at every rung plus the new 3x step (old: 24 / 12 / 6 s per day). The hours-per-real-second multiplier is **derived** (`TimeManager.hours_per_real_second(idx) = HOURS_PER_DAY / SECONDS_PER_DAY[idx]`, 0.0 for pause and out-of-range; named to avoid collision with the unrelated `HROvertimeSystem.speed_multiplier` build-throughput helper), so the array is the only place tempo is expressed; `SPEED_MULTIPLIERS` is gone. Constants stay in `time_manager.gd` beside `HOURS_PER_DAY` / `INITIAL_HOUR` rather than moving to a new `TimeConstants` file — they were already a single home, and the two constants are dimensionally tied. **Index remap:** 4x moved from speed index 3 → 4; safe because nothing emitted a literal `3`, every restore path uses a captured value or `last_running_speed`, and speed is not persisted in `Settings`/`SaveManager`. `TopBar.tscn` gains `Speed3Btn` (array index == speed index, so highlight/round-trip need no new code); no theme or localization work (`"3x"` is a language-neutral numeral). New keyboard shortcuts **1/2/3/4** select running speeds in `game_shell.gd:_input`, behind the two guards Space already used (focused text field; a modal owning pause) — the modal guard is what keeps them distinct from the 1-4 dialogue-choice keys. The seven `_pre_*_speed < 0` restore fallbacks in `main.gd` now resolve to `TimeManager.last_running_speed` instead of a hardcoded 1x. Day 1 is unchanged at 15 in-game hours (7.5s at 1x). Verified: `speed_ladder` + `speed_day_invariant` smoke cases, and a `--tempo-probe=<idx>` stopwatch harness measuring 11992-12006 / 5996-6003 / 2995-3002 / 1497-1504 ms per day. | The old 24s/day at 1x sat between every useful pace — too fast to read an event modal, far too slow for quiet stretches and for F5 testing (a 180-day run was 72 min of pure clock time before a single decision was read). `CLAUDE.md` Calibration Law 2 names this exact grammar as the target: "Fast 4x that compresses dead days + auto-slow when a decision arrives." Seconds-per-day is the honest unit for a tempo decision (it is what a designer stopwatches) whereas an abstract multiplier hides the real pace; deriving the multiplier from it means a retune is a one-array edit. 3x fills the gap between "cruising" and "skip the quiet stretch" that the 2x→4x jump left. Values are WORKING per calibration §10 (numbers last); the ladder *shape* is the locked part. Consequence accepted: a 180-day run is now 36 / 18 / 9 / 4.5 min of pure clock time, so the demo's 60-90 min target leans much more on paused decision time. |

| 2026-08-08 | **Resolution is DETECTED, not defaulted; and the ODA room gets a crop budget.** Three linked decisions. **(1) Native detection.** `DisplaySettings.native_resolution()` reads `screen_get_size(window_get_current_screen())` — the monitor the window actually opened on, not the primary — and `usable_size()` reads `screen_get_usable_rect()` so a windowed default is never pushed under the taskbar. `default_resolution()` picks the largest supported mode that fits the usable area **and matches the panel's aspect ratio**; aspect beats area, because raw largest-area chose 2560×1080 (21:9) on a 2560×1440 panel. `get_resolution()` returns that computed value whenever the key is absent, so first run and "reset to defaults" both re-detect (`Settings.has_stored()` is the new seam; the `DEFAULTS` literals are now only the headless fallback). The settings row shows `effective_resolution()` — native while a fullscreen mode is active, the stored choice while windowed — because the disabled row previously read "1920 × 1080" while the game ran at 2560×1440. **(2) The list is additive.** `RESOLUTIONS` gains the 16:10, 32:9 and UWQHD+ modes §14.1 already claimed, and `available_resolutions()` now *inserts* the detected native rather than only filtering the table down, so laptop and non-standard panels are selectable. Fixed alongside: an unlisted stored value made the dropdown silently display `1280 × 720` without writing it back, so the first click dropped the player to 720p. **(3) Boot mode.** `project.godot` gains `window/size/mode=3` and zeroed `window_*_override` so the first frame is already borderless-fullscreen; the engine creates the window before any autoload runs, so the old 1920×1080 bordered first frame was a visible flash. `viewport_width/height` are untouched and must stay so — they are the canvas_items denominator, mirrored by hand in `DisplaySettings.BASE_VIEWPORT`. **(4) ODA crop budget.** `OdaLayout.MAX_CROP = 0.02` caps how much of the 16:9 room painting `KEEP_ASPECT_COVERED` may crop vertically; beyond it the room is drawn as a centred band with a `BG_ART` surround. Expressed as a crop budget rather than a fixed aspect so the transition is gradual instead of a cliff. | Measured, not assumed: at 5120×1440 the visible band collapsed to [0.261, 0.739] and the three picture frames, the papers **and the phone** were entirely off-screen — all clickable anchors, and the phone is the mentor/event surface; at 3440×1440 the frames were cut in half. Because every anchor, every host-derived surface and `visible_band_y` all derive from `cover_transform`, capping the view inside that one function moved all of them together with no call-site changes. The budget is 0.02 rather than the data-derived 0.012 (PAPER_SLOTS' third slot ends at 0.988) because 1920×1080's CenterViewport is already ~1.851 and crops 1.94% — tightening to 0.012 would have put letterbox bars on the **primary** resolution to fix a paper slot that is already hair-clipped today, a strictly larger regression. That paper margin belongs to the polish wave. Guarded by the `oda_anchors_stay_in_band` smoke case, which was written red (it failed on the pre-fix `maxf` cover), then green, then re-falsified. |
| 2026-08-19 | **Calibration Round A (post-localization, the full package).** One worktree branch, one commit per section, every number proven with a before/after `--run-log` pair (seed 424242) and a falsification. **(1) Quality scale:** `QualityModel.NORMALIZE_HALF_SAT` 50→25 TOGETHER WITH the B2B tolerance band (35,5)→(33,9) [ÖLÇ: measured T_good 55 / T_bad 42, smallest PER meeting the 60 %/20 % book fractions], `saas_ops_field` research unlock, `SalesSystem.SATISFACTION_QUALITY_GATE` 70→40 on the EXPERIENCE axis (director ruling), and a rival scale bridge (`RIVAL_TEMPLATE_HALF_SAT` 50 — the rival table is authored on the retired grown scale; its re-authoring is Layer B). **(2) Goal-terminated run:** `RUN_END_DAY`/the Day-180 fork/`net_history_90`/`cash_went_negative` retired; `EndingsSystem.SOFT_CAP_DAY` 730 → `running_on_fumes` (id kept per the Y5.6 ruling; paper rewritten, title "İlgi Söndü / Interest Faded"; not deferred for a live sheet — ledger 16; D-1 warning retargeted). **(3) Calendar-month ledger + Series A gate:** `GameState.month_history` (12 closes, int-only entries, closed by MonthSummarySystem on the 1st before the recap; one-day lag deliberate); condition `mrr_growth_streak`; gate 2→3 = revenue bar + 3 months ≥12 % MoM + brand; `TRACTION_MRR_TARGET` 5,000→40,000 [ÖLÇ rule → band floor; the played B2B curve plateaus at $39K]; the NUMBER is never rendered — `PhaseGateSystem.series_a_signal()` (closed/warming/open) is the single home the Finance indicator, product strip and ODA goal card paint from (`InvestorAppetiteUi`); `traction_progress` retired. **(4) B2C compounding:** word-of-mouth terms on the aggregate's satisfaction (`WOM_COEF` 0.005 [ÖLÇ sweep], gate 60, base multiplier floor 0.3). **(5) Bugs hit conversion** (`BUG_CONV_COEF` 0.02/bug, floor 0.4). **(6) Cost types:** `ev_ps_bug_complaint` costs audience/satisfaction, never cash; `audience_delta{pct}` modifier + `EFFECT_AUDIENCE_PCT` badge. **(7) Retention:** `RETAIN_DISCOUNT_MAX_USES` 2 (both channels), `RISK_REENTRY_DAYS` 21 hysteresis, locked-visible discount row (`b2b_discounts_below` condition + `B2B_DISCOUNT_SPENT_DESC`), ONE gate `can_offer_retention` for the sweep and the Sales-tab button. **(8) Profitability is a CONDITION:** 6 consecutive Artıda closes (net>0, no red day) + margin ≥15 % + MRR ≥ $20K (decoupled from the Series A bar); Finance "Artıda · n/6 ay". **(9) Speed ladder 1×/2×/3×:** `SECONDS_PER_DAY` [0,12,6,3], KEY_4 inert, TopBar `Speed4Btn` removed, stored 4 clamps to 3. **(10) Harness:** smoke seed pinned (424242), `--run-log` joins the autosave sniffer, ambient rolls use the exact per-hour chance 1−(1−p)^(1/n), S2-33 draft guard (preserve: `creation_draft` flag + `center_viewport` propagate_call). **(11) Strings:** `SET_RESOLUTION_BORDERLESS` landed; every new label TR+EN in the same commit. Report: `docs/audits/calibration_round_A_2026-08-19.md`. | Three evidence documents agreed the post-ship loop failed for calibration reasons before design reasons (a played v1 born 15-30 points under every account; a 180-day wall; a $5K Series A bar; a fixed-point B2C formula; an uncapped discount). Until these moved, no playtest read a true baseline and the design session had no curve to sit on. Measured result of the package (full_run:730:sim): retention modals 630→43, discount answers 619→4, modals/week 25.8→1.3, peak MRR $7,349→$39,050, ending `profitable_bootstrap` at month 9; the Series A bar is not crossed inside 24 months (the B2B prospect pool is catalog-bound) — reported as the revenue-curve finding for Layer B, the bar not lowered (director ruling). |

| 2026-08-20 | **Delivery model: one folder, one branch (§19.7).** Branches, worktrees and merges are retired. Work is committed straight to `main` in the single checkout; the remote carries only `main`. Review is Erdem opening the game — "geri al" reverts, "push et" pushes — so no branch is needed to park work pending approval. **This supersedes the "one worktree branch, one commit per section" method described in the 2026-08-19 row above**, which stays as history. The 2026-08-19 recipe of a temporary `git worktree add --detach` to re-take a theme baseline is retired with it: the baseline dump is taken in the same tree before editing. | Three finished sessions (Localization Phase 2, Calibration Round A, Build Bar) sat outside `main` for weeks, so the game Erdem playtested contained almost none of what had been built, and "the game" named three codebases that disagreed. The merge that reconciled them found no ruling conflicts at all — the branches had cost isolation without buying review. Parallel worktrees also fed a whole class of failure: a shared `.godot/` class cache, a shared `user://settings.json` that moved the ODA theme audit, and unowned uncommitted edits. A revert is cheaper than a branch when the reviewer is one person looking at one build. |
| 2026-08-22 | **Assignment moves from JOBS to AREAS; save schema 4 → 5. Trait count becomes exactly one. `AREA_MAX` 9 → 10 so the five-star ruler can fill.** The seven jobs of 18d27e3 (`build` · `test` · `support` · `accounts` · `sales` · `research` · `cost`) are retired; `HRConstants.ASSIGNABLE` is the six areas plus `research`, and a person may hold only their KEY or SECONDARY area (`can_hold_area`) — the founder may hold any one of the seven. `support` and `accounts` collapse onto Müşteri İlişkileri, which is what ch. 06 §1.3's "covering head" was counting anyway. `TRAIT_COUNT := 1`: everyone carries exactly one trait, positive OR negative, so a candidate can be a pure liability. Stars are `points / 2` with real halves; `AREA_TRAIN_CAP` stays 8, so paid training tops out at four stars and the fifth comes only from doing the work or hiring a premium file (`BAND_SHAPE.senior` peak 9 → 10). | GDD v2 ch. 07 rev 2 §4 says "alanlar işaretlenir" while its table is headed "İş"; the first pass read the heading, the **approved skin** (Unicorn Skins, turns 9-11, DIRECTOR-APPROVED 2026-08-22) settles it by drawing the Görevler matrix with one column per AREA and naming Build/Destek/Hesap/Maliyet as retired. Every consumer got SIMPLER, not more complex: `_phase_area_sum` now reads "everyone assigned to this phase's area" instead of "everyone on the build job, then work out which area they'd use". The visible consequence is deliberate and worth naming: **a person contributes only where they are assigned, and can only be assigned to two areas** — so a developer can no longer help the design phase at all. §2's "tek kişilik ekipte boşluk kalmaz" still holds, but about SKILLS (everyone carries six numbers), not about coverage. Two live bugs fell out of the new smoke cases: a founder sent to training never came back (`HRSystem.tick_training` walked employees only) and the v4→v5 lead-seat remap resolved `build` against an empty stats dict, moving every lead to Ürün. A third defect was caught by EYE, not by a gate, and the gate was then tightened: a role-hint value carried an UNQUOTED COMMA, so the CSV split into four columns — TR kept only the first fragment and the EN column received TR's second fragment, which is how an English player got a Turkish sentence on screen. `loc_csv_integrity` asserted `row.size() < 3` and let it through; it now asserts exactly 3. |
| 2026-08-21 | **Skill AREAS replace the three employee axes and the five founder skills; save schema 3 → 4.** `expertise`/`pace`/`rapport` and `tech`/`sales`/`negotiation`/`leadership`/`influence` all became one vocabulary: six areas (`product` · `design` · `engineering` · `qa` · `sales` · `customer_success`) carried by EVERYONE, plus `leadership` on everyone and `charisma` on the founder alone. Hız is deleted outright; Uyum stops being a number and moves into traits; Karizma returns under its own name and leaves `FounderConstants.OLD_SKILLS`. A new **assignment layer** sits beside it: `Character.assigned_jobs` over the seven jobs of ch. 07 rev 2 §4, with the founder hard-locked to one (ch. 02 §5) and employees paying `OVERLOAD_*` for more. `SaveManager._migrate_character_areas` is the first character migration the project has needed. | GDD v2 ch. 07 **rev 2**, DIRECTOR-APPROVED 2026-08-21. The old axes had identical keys for every role and only their MEANING changed per role, so a one-person team had holes nothing could fill; areas close that (§2: "tek kişilik ekipte boşluk kalmaz") and §5 is its price. The migration was unavoidable rather than optional because `SaveCodec.res_from_dict` replaces `role_stats` WHOLESALE — an old save's three keys would have loaded intact and then failed the key lock on every employee, producing a run that loads, looks fine and reads 0 for every skill. Salaries deliberately did not move: `_shape_premium` still prices the 3-long `BAND_SHAPE`, which is what its whole invariant table is written about. Three formula bindings are inferences rather than quotations (İnovasyon→Ürün, Stabilite→Yazılım, and the term-sheet dilution lever→Karizma); the first two were approved with the plan, the third belongs to ch. 09's turn and is one token in `PitchConstants.LEVER_SKILL`. |
