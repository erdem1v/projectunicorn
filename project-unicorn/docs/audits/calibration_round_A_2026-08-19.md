...
7. **Merge notes vs the Build Bar session.** The main checkout carries uncommitted edits to `event_manager.gd`, `event_modal.gd`, `product_system.gd`, `strings.csv` ("Build Bar 2026-08-19": retires `advance_iteration`, adds `enter_beta`) and `display_settings.gd`, plus untracked `docs/.md`, `docs/content/events_draft/`. This branch touches `event_manager.gd` (conditions + `hourly_chance`, `_is_eligible` signature), `event_modal.gd` (`_describe_modifier` audience pct) and `strings.csv` (25 new + rewritten rows) → expect textual merges in those three; RunProbe's Beta wait calls no iteration API, but a retired `advance_iteration` would touch the `endgame_smoke.gd` cases that still call it — run the suite once after the merge.
