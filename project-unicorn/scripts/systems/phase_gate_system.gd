...
# Phase Transition Engine — daily tick slot 8.
#
...
#   · the two shutter early-returns plus on_shutter_cleared to undo them. One
...
# Series A gate — MRR ONLY (director decision). The revenue bar is
# SalesSystem.TRACTION_MRR_TARGET (never rendered as a figure — director ruling: the signal is
# shown, the number is not). The growth streak and the brand floor that used to
# sit on this gate are GONE from it: growth still sets the valuation multiple (PitchConstants
# ARR_GROWTH_* bands) and brand still moves conviction in the meeting, but neither holds the
# door. What the player reads is series_a_signal() below — a chip, no bar, no "n/3".
#
...
# Gate table. Conditions are written in the ENGINE's condition vocabulary and are
...
# working placeholders (numbers last).
const GATES := [
	{
		"from": 1, "to": 2,
	...
	},
	{
		...
		"conditions": [
			...
			# (series_a_signal paints a state, not the number). THE ONLY LEAF: the
...
static func _open_gate(gate: Dictionary) -> void:
	...
	GameState.submit_month_highlight(TranslationServer.translate("GATE_OPENED").format(
		{"phase": _phase_display_name(int(gate.to))}), 70)  # AYIN OLAYI
	...
	# The shutter hold is the card's `finance.shutter_days_left < 0` leaf.


static func on_gate_declined() -> void:
	# "Henüz değil" — no penalty; escalation index advances, reminder
