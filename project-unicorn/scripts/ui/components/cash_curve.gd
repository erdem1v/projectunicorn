...
const DASH_LEN := 5.0
# İKİ PROJEKSİYONUN AYRI RİTMİ: kesik boyu renkten BAĞIMSIZ ikinci bir işarettir; renk körü
# modunda mavi/turuncu ayrımı da okunur, ama grafik yalnız hue'ya dayanmaz.
...
func _draw() -> void:
	...
	if samples.is_empty():
		return  # set_data henüz gelmedi
	var today_day: int = int(_d.today_day)
	...
	var x1: float = size.x - PAD_RIGHT
	var y0: float = PAD_TOP
	var y1: float = size.y - PAD_BOTTOM
	...
	cash_max *= 1.08   # cash_min ≤ 0 < 1 ≤ cash_max, yani aralık hiç sıfır olmaz
	...
	# --- 3. Gerçekleşen çizgi — DOLGU YOK (Terminal mockup'ı). Tek örnek → yalnız nokta
	...
	# --- 4. Bugün: kesikli dikey hairline; projeksiyonlar buradan çatallanır ---
