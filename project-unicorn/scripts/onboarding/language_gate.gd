...
# current step to re-mount, and the half-filled draft to survive that re-mount.
#
...
#
# The OS locale only PRESELECTS (focus). The click is always explicit: guessing silently
# is how a player ends up in a language they did not choose and cannot navigate out of.
...
func _ready() -> void:
	...
	var tr_btn := _make_option("Türkçe", "tr")
	var en_btn := _make_option("English", "en")
	_buttons.add_child(tr_btn)
	_buttons.add_child(en_btn)
	# Preselect from the OS, focus only — never auto-advance.
	(tr_btn if OS.get_locale_language() == "tr" else en_btn).grab_focus()
