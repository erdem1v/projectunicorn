...
# A Series A sheet lives in GameState.active_sheets (max PitchConstants.MAX_SHEETS), made by
# VCPitchSystem on a won meeting; the seed offer lives in GameState.seed_sheet, made by
# SeedRoundSystem.make_seed_sheet. Both use TermSheet.new() + field assignment, never a
# constructor — the project Resource convention. The Term Sheet Table opens from
# opening_terms; leverage is DERIVED (not stored) so it can never go stale — computed
# against the live sheet set at table time.
#
...
## The table's NUMERIC opening offer, fixed at grant. Series A: {valuation_m, dilution_pct,
## board_seats, board_veto}, priced from the run (VCPitchSystem._derive_series_a_terms); seed:
## {raise, dilution_pct, board_seats, board_veto} (SeedRoundSystem.make_seed_sheet). Pre-close
## it is shown NOWHERE but the table (valuation is a table-only secret).
